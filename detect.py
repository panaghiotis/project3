...
for time_series in range(num):
    ...
    test_score_df = pd.DataFrame(index=col_names[(data_to_be_trained + TIME_STEPS):])
    ...
    plt.plot(col_names[(data_to_be_trained + TIME_STEPS):], test_list[time_series][TIME_STEPS:], label='time series value')
